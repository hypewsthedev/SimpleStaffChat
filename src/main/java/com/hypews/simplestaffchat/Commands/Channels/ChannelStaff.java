...
public class ChannelStaff implements CommandExecutor {
    Plugin plugin = SimpleStaffChat.getPlugin(SimpleStaffChat.class);
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        Player p = (Player) sender;
        if (!p.hasPermission("staffchat.staffchannel")) {
            UtilUtilities.msg(p, UtilMessages.NO_PERMS);
        }
        String message = plugin.getConfig().getString(ChatColor.translateAlternateColorCodes('&', "staffChannelPrefix")) + " " + p.getName() + "&7:&r ";
        for (String s : args) {
            message = message + s + " ";
        }
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (player.hasPermission("staffchat.staffchannel")) {
