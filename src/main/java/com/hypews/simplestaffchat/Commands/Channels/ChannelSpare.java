...
public class ChannelSpare implements CommandExecutor {
    Plugin plugin = SimpleStaffChat.getPlugin(SimpleStaffChat.class);
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        Player p = (Player) sender;
        if (!p.hasPermission("staffchat.sparechannel")) {
            UtilUtilities.msg(p, UtilMessages.NO_PERMS);
        }
        String message = plugin.getConfig().getString(ChatColor.translateAlternateColorCodes('&', "spareChannelPrefix")) + " " + p.getName() + "&7:&r ";
        for (String s : args) {
            message = message + s + " ";
        }
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (player.hasPermission("staffchat.sparechannel")) {
